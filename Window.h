...
Window::Window(int width, int height, string name) { //constructor for welcome window
    this->name = name;
    this->width = width;
    this->height = height;
    this->page = -1;
        ...
        while (redBlack.pollEvent(event))
        {
            ...
            if (event.type == sf::Event::KeyPressed){
                ...
                if(event.key.code == sf::Keyboard::Enter && input.length() > 0){
                    ...
                    sf::Clock clock;
                    ////do red black search
                    vector<string> artists(35,"Red Black Name");
                    vector<string> familiars(35, "0.123");
                    vector<string> songs(35, "XC54637294LKZJMSKY");
                    vector<string> year(35, "3005");
                    vector<vector<string>> yes;
                    yes.push_back(artists);
                    yes.push_back(familiars);
                    yes.push_back(songs);
                    yes.push_back(year);
                    ////
        ...
        while (hash.pollEvent(event))
        {
            ...
            if (event.type == sf::Event::KeyPressed){
                ...
                if(event.key.code == sf::Keyboard::Enter && input.length() > 0){
                    ...
                    sf::Clock clock;
                    ////do hash map search
                    vector<string> artists(35,"Hash Name");
                    vector<string> familiars(35, "0.123");
                    vector<string> songs(35, "XC54637294LKZJMSKY");
                    vector<string> year(35, "3005");
                    vector<vector<string>> yes;
